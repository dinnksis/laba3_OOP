...
int main(){
    ...
    std::cout<<archive1.use_count()<<std::endl;
    {
        ...
        std::weak_ptr<archive> weak_archive1=archive1;
        std::cout<<archive1.use_count()<<std::endl;
    }
    std::weak_ptr<archive> weak=archive1;
    std::cout<<"not in block: "<<archive1.use_count()<<std::endl;
    if (auto archiveweak = weak.lock()) {
        std::cout << "count with weak_ptr:  " << archiveweak.use_count() << std::endl;
    } else {
        std::cout << "weak_ptr deleted :(" << std::endl;
    }
